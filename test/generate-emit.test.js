...
import { parsesCleanly } from "./generate-helpers.js";
...
test("emitSceneFile imports only what the chosen stubs need", () => {
  const out = emitSceneFile({ className: "Level2", lifecycle: ["onInitialize", "onPreLoad", "onActivate"] });
  ...
  const minimal = emitSceneFile({ className: "A", lifecycle: [] });
