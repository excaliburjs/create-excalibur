...
import { callTool, listTools } from "../src/mcp/registry.ts";
import { scaffoldProject, ScaffoldError } from "../src/create/scaffold.ts";
import { HIGHLIGHT_POST, HIGHLIGHT_PRE } from "../src/docs/algolia.ts";
import { manifestPath, writeCachedFile, writeJsonAtomic } from "../src/docs/cache.ts";
import { buildPageList } from "../src/docs/fetch-docs.ts";
import { buildIndex, saveIndex } from "../src/docs/local-index.ts";
import { buildPluginIndex, pluginEntryFromRegistryDoc, pluginsManifestPath, savePluginIndex, writePluginReadme } from "../src/docs/plugins.ts";
import { withViteProject, ts, read, parsesCleanly } from "./generate-helpers.ts";
import { withDoctorProject } from "./doctor-helpers.ts";
import { FIXTURES, readFixture, withTempHome, withTempDirAsync } from "./helpers.ts";
...
test("analyze_project reports scenes and resources without leaking ts", async () => {
  await withViteProject(async ({ dir }) => {
    ...
    assert.deepEqual(info.scenes.map((s: { className: string; key?: string }) => [s.className, s.key]), [["MyLevel", "start"]]);
...
test("generate_resource derives the key, warns on missing files, rejects duplicates", async () => {
  await withViteProject(async ({ dir }) => {
    ...
    assert.ok(report.warnings.some((w: string) => w.includes("not found under")), "missing-file warning");
...
test("list_templates returns unique deduped ids", async () => {
  ...
  assert.ok(result.templates.every((t: { repo?: string }) => t.repo?.startsWith("https://")));
});
...
test("scaffoldProject clones, cleans, and reports (injected clone)", async () => {
  await withTempDirAsync(async (dir: string) => {
    ...
    const clone = async (_repo: string, target: string) => {
...
test("doctor reports findings for a seeded project", async () => {
  await withDoctorProject(
    async ({ dir }) => {
      ...
      assert.deepEqual(
        result.findings.map((f: { rule: string }) => f.rule),
...
test("upgrade tool plans and applies with the git gate surfaced as isError", async () => {
  const { withUpgradeProject, ts: upgradeTs } = await import("./upgrade-helpers.ts");
  await withUpgradeProject(
    async ({ dir }) => {
      ...
      assert.ok(dry.plan.some((p: { id: string }) => p.id === "goto-to-gotoscene"));
      ...
      assert.ok(applied.applied.some((a: { id: string }) => a.id === "goto-to-gotoscene"));
