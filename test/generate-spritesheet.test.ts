...
import { applyAnimation, applySpriteSheet } from "../src/generate/apply.ts";
import { emitAnimationConst, emitSpriteSheetConst } from "../src/generate/emit.ts";
import { GenerateError } from "../src/generate/errors.ts";
import { analyzeProject } from "../src/generate/project.ts";
import { createTsEditor } from "../src/generate/ts-edit.ts";
import { resolveGrid } from "../src/generate/wizards-sprite.ts";
import { callTool } from "../src/mcp/registry.ts";
import { parsesCleanly, read, ts, withViteProject } from "./generate-helpers.ts";
...
function editorFor(text: string, name = "resources.ts") {
...
function payload(result: object) {
  const r = result as { isError?: boolean; content: Array<{ text: string }> };
  assert.ok(!r.isError, `unexpected tool error: ${r.content?.[0]?.text}`);
  return JSON.parse(r.content[0].text);
}

function errorText(result: object) {
  const r = result as { isError?: boolean; content: Array<{ text: string }> };
  assert.equal(r.isError, true, "expected an isError result");
...
test("generate_animation builds frames, checks bounds, and wires an actor", async () => {
  await withViteProject(async ({ dir }) => {
    ...
    assert.ok(result.modified.some((m: { path: string }) => m.path === "src/player.ts" || m.path === path.join("src", "player.ts")));
    const out = read(dir, "src/resources.ts");
