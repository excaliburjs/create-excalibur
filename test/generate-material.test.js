...
import { applyMaterial } from "../src/generate/apply.js";
import { MATERIAL_TEMPLATES, emitMaterialFile, materialNames } from "../src/generate/emit.js";
import { GenerateError } from "../src/generate/errors.js";
import { pickActor } from "../src/generate/wizards.js";
import { parsesCleanly, read, withViteProject } from "./generate-helpers.js";
...
test("pickActor resolves --actor prompt-free and errors helpfully on a miss", async () => {
  await withViteProject(async ({ project }) => {
    ...
    await assert.rejects(
      ...
      (err) => err instanceof GenerateError && /available actors: Player/.test(err.hint)
    );
  });
});
