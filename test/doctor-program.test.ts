...
import { createProgramContext } from "../src/doctor/program.ts";
import { runDoctor } from "../src/doctor/run.ts";
import { GenerateError } from "../src/generate/errors.ts";
import { withViteProject } from "./generate-helpers.ts";
import { withDoctorProject, ts } from "./doctor-helpers.ts";
...
test("a ts module without the full compiler API is rejected with a typescript@6 hint", () => {
  const fakeTs = { version: "9.9.9", createSourceFile() {} } as unknown as typeof ts;
  assert.throws(
