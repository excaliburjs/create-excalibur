...
import { applyActor, applyAnimation, applyEngine, applyLabel, applyMaterial, applyResource, applyScene, applySpriteSheet, applyUpdateActor } from "../../generate/apply.ts";
import { MATERIAL_TEMPLATES, SCENE_LIFECYCLE_METHODS } from "../../generate/emit.ts";
import { GenerateError } from "../../generate/errors.ts";
import { isValidIdentifier, toCamelCase, toPascalCase } from "../../generate/names.ts";
import { analyzeProject } from "../../generate/project.ts";
import { COLORS, DISPLAY_MODES, RESOURCE_TYPES, detectPixelArt, pickActor, pickScene, resolveName } from "../../generate/wizards.ts";
import { ANIMATION_STRATEGIES, assetDiskPath, resolveGrid } from "../../generate/wizards-sprite.ts";
import { readImageSize } from "../../generate/image.ts";
import { jsonResult } from "../result.ts";
import { resolveProjectDir } from "./docs.ts";
import type { JsonSchema, Tool, ToolContext, ToolResult } from "../types.ts";
import type { Project, ProjectSpriteSheet } from "../../generate/project.ts";
import type {
...
import type { ImageSize } from "../../generate/image.ts";

interface CommonWriteArgs {
  dryRun?: boolean;
  force?: boolean;
  projectDir?: string;
}

interface GenerateActorArgs extends CommonWriteArgs {
  name: string;
  scene?: string;
  collider?: { type: "box" | "circle" | "custom" | "none"; width?: number; height?: number; radius?: number };
  graphic?: { type: "color" | "sprite" | "none"; color?: string; resourceKey?: string };
  collisionType?: string;
  pos?: Vec2;
  z?: number;
  anchor?: "center" | "topLeft";
  coordPlane?: "World" | "Screen";
  rotation?: number;
  collisionGroupName?: string;
}

interface GenerateLabelArgs extends CommonWriteArgs {
  name: string;
  scene?: string;
  text?: string;
  font?: { family?: string; size?: number; bold?: boolean; color?: string };
  pos?: Vec2;
}

interface GenerateSceneArgs extends CommonWriteArgs {
  name: string;
  lifecycle?: string[];
  register?: boolean;
  key?: string;
...
interface GenerateResourceArgs extends CommonWriteArgs {
  type: string;
  assetPath: string;
  key?: string;
  pixelFiltering?: boolean;
  family?: string;
  responseType?: string;
  scene?: string;
}

interface GenerateMaterialArgs extends CommonWriteArgs {
  name: string;
  template?: string;
...
interface UpdateActorArgs extends CommonWriteArgs {
  actor: string;
  options?: Record<string, unknown>;
  remove?: string[];
}

interface UpdateEngineArgs extends CommonWriteArgs {
  options?: Record<string, unknown>;
  remove?: string[];
}

interface GenerateSpriteSheetArgs extends CommonWriteArgs {
  name: string;
  assetPath?: string;
  resourceKey?: string;
  rows?: number;
...
interface GenerateAnimationArgs extends CommonWriteArgs {
  name: string;
  spriteSheet?: string;
  frames: Array<{ x: number; y: number; duration?: number }>;
  duration?: number;
  strategy?: string;
  actor?: string;
}

export const PROJECT_DIR_PROP: Record<string, JsonSchema> = {
  projectDir: {
    type: "string",
...
const COMMON_WRITE_PROPS: Record<string, JsonSchema> = {
...
const SCENE_PROP: Record<string, JsonSchema> = {
  scene: {
    type: "string",
...
async function loadProject(args: { projectDir?: string }, ctx: ToolContext): Promise<Project> {
  const projectDir = resolveProjectDir(args, ctx);
  return analyzeProject(projectDir, ctx.ts ? { ts: ctx.ts } : {});
}

function writeOpts(args: CommonWriteArgs) {
  return { dryRun: args.dryRun ?? false, force: args.force ?? false };
}

async function resolveTargetScene(args: { scene?: string }, project: Project) {
  if (!args.scene) return null;
  return pickScene({ project, sceneArg: args.scene });
}

function report(result: GenerateReport, args: CommonWriteArgs): ToolResult {
  return jsonResult({ dryRun: args.dryRun ?? false, ...result });
}

export const generateTools: [
  Tool<{ projectDir?: string }>,
  Tool<GenerateActorArgs>,
  Tool<GenerateLabelArgs>,
  Tool<GenerateSceneArgs>,
  Tool<GenerateResourceArgs>,
  Tool<GenerateMaterialArgs>,
  Tool<UpdateActorArgs>,
  Tool<UpdateEngineArgs>,
  Tool<GenerateSpriteSheetArgs>,
  Tool<GenerateAnimationArgs>,
] = [
  {
    ...
    async handler(args, ctx) {
      ...
      const rel = (f: string | null | undefined) => (f ? path.relative(project.projectDir, f).split(path.sep).join("/") : null);
      return jsonResult({
  ...
  },
  {
    ...
    async handler(args, ctx) {
      ...
      const colliderArg = args.collider ?? { type: "box" as const, width: 100, height: 100 };
      let collider: ColliderModel;
      if (colliderArg.type === "box") {
        collider = { type: "box", width: colliderArg.width ?? 100, height: colliderArg.height ?? 100 };
      } else if (colliderArg.type === "circle") {
        collider = { type: "circle", radius: colliderArg.radius ?? 50 };
      } else {
        collider = { type: colliderArg.type };
      }

      const colorable = collider.type === "box" || collider.type === "circle";
      const graphicArg = args.graphic ?? (colorable ? { type: "color" as const, color: "ExcaliburBlue" } : { type: "none" as const });
      let graphic: GraphicModel;
      if (graphicArg.type === "color") {
        if (!colorable) {
        ...
        graphic = { type: "color", color: graphicArg.color ?? "ExcaliburBlue" };
      } else if (graphicArg.type === "sprite") {
        if (!graphicArg.resourceKey) {
        ...
        if (!project.resourceKeys.includes(graphicArg.resourceKey)) {
          throw new GenerateError(`resource key "${graphicArg.resourceKey}" not found in Resources.`, {
            hint: project.resourceKeys.length
              ? `available resource keys: ${project.resourceKeys.join(", ")}`
              : "no Resources found — add one with generate_resource first.",
          });
        }
        graphic = { type: "sprite", resourceKey: graphicArg.resourceKey };
      } else {
        graphic = { type: "none" };
      }
  ...
  },
  {
    ...
    async handler(args, ctx) {
      ...
      const model: ResourceModel = { key, resourceClass: spec.class, assetPath: args.assetPath };
  ...
  },
  {
    ...
    async handler(args, ctx) {
      ...
      const model = { actor, options, remove };
  ...
  },
  {
    ...
    async handler(args, ctx) {
      ...
      const project = await loadProject(args, ctx);
      const model = { options, remove, scenes: project.scenes };
  ...
  },
  {
    ...
    async handler(args, ctx) {
      ...
      if (args.resourceKey) {
        ...
        const key = args.key ?? toPascalCase(path.basename(assetPath, path.extname(assetPath)));
        if (!isValidIdentifier(key)) {
      ...
      const model: SpriteSheetModel & { dimensions: ImageSize | null } = {
        name,
        image,
        dimensions,
        grid,
  ...
  },
  {
    ...
    async handler(args, ctx) {
      ...
      let sheet: ProjectSpriteSheet;
      if (args.spriteSheet) {
        const found = sheets.find((s) => s.name === args.spriteSheet);
        if (!found) {
      ...
      if (grid) {
        const bad = frames.find((f) => f.x >= grid.columns || f.y >= grid.rows);
        if (bad) {
          throw new GenerateError(`frame ${bad.x},${bad.y} is outside the sheet's grid.`, {
            hint: `${sheet.name} is ${grid.columns} columns \u00d7 ${grid.rows} rows (0-based).`,
          });
        }
      }
