...
import { plainSnippet } from "../../docs/algolia.ts";
import { docsCacheRoot } from "../../docs/cache.ts";
import { DocsError } from "../../docs/errors.ts";
import { syncApiSymbols, syncDocs } from "../../docs/fetch-docs.ts";
import { loadPage, pageSectionMarkdown } from "../../docs/page.ts";
import { loadPluginPage, syncPlugins } from "../../docs/plugins.ts";
import { runDocsSearch } from "../../docs/search.ts";
import { detectExcaliburVersion, refForVersion } from "../../docs/version.ts";
import { jsonResult, textResult } from "../result.ts";
import type { JsonSchema, Tool, ToolContext } from "../types.ts";
import type { HitKind } from "../../docs/types.ts";
import type { DocsPage } from "../../docs/page.ts";

const PROJECT_DIR_PROP: Record<string, JsonSchema> = {
  projectDir: {
    type: "string",
...
const REF_PROP: Record<string, JsonSchema> = {
  ref: {
    type: "string",
...
export function resolveProjectDir(args: { projectDir?: string | null }, ctx: ToolContext): string {
  return path.resolve(ctx.defaultProjectDir, args.projectDir ?? ".");
}

function resolveRef(args: { ref?: string | null; projectDir?: string | null }, ctx: ToolContext): string {
...
function toolifyHint(error: unknown): unknown {
...
export const docsTools: [Tool<DocsSearchArgs>, Tool<DocsGetPageArgs>, Tool<DocsSyncArgs>] = [
  {
    name: "docs_search",
    description:
    ...
    async handler(args, ctx) {
      ...
      const payload: { source: string; ref: string; hits: unknown[]; fallback?: string } = { source: result.source, ref, hits };
    ...
    async handler(args, ctx) {
      ...
      if (args.apiSymbols ?? true) {
        await syncApiSymbols().catch((error) => {
          warnings.push(`API symbol sync failed: ${message(error)}`);
        });
      }
      let plugins: number | null = null;
      if (args.plugins ?? true) {
        try {
          ...
          warnings.push(`Plugin README sync failed: ${message(error)}`);
        }
      }
