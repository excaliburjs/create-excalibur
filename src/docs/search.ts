...
import { hasIndex } from "./cache.ts";
import { DocsError, DocsNetworkError } from "./errors.ts";
import { searchLocal } from "./local-index.ts";
import { hasPluginIndex, searchPlugins } from "./plugins.ts";
import type { DocsHit, HitKind, HitSource } from "./types.ts";

export interface DocsSearchOptions {
  query: string;
  /** cache ref for the offline index (e.g. "v0.32.0" or "main") */
  ref: string;
  limit?: number;
  /** filter; "plugin" searches the offline plugin-README index */
  kind?: HitKind | null;
  /** force the offline index */
  offline?: boolean;
 ...
 */
export async function runDocsSearch({ query, ref, limit, kind = null, offline = false, signal = null }: DocsSearchOptions): Promise<DocsSearchResult> {
 ...
 */
function withPluginHits(result: DocsSearchResult, query: string, kind: HitKind | null, limit: number = DEFAULT_LIMIT): DocsSearchResult {
