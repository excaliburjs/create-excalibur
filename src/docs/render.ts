...
export function supportsHyperlinks(stream: { isTTY?: boolean } = process.stdout, env: NodeJS.ProcessEnv = process.env): boolean {
...
// ---------------------------------------------------------------------------
export function wrap(text: string, width: number, { indent = "", firstIndent = indent }: { indent?: string; firstIndent?: string } = {}): string {
  const lines: string[] = [];
...
export function highlightCode(code: string, lang: string | undefined, c: ChalkInstance): string {
  if (!c.level || !/^(ts|typescript|js|javascript|jsx|tsx|json|mjs|cjs)$/.test(lang ?? "")) {
    return code;
  }
  return code.replace(CODE_TOKEN_RE, (m, comment: string | undefined, str: string | undefined, num: string | undefined, ident: string | undefined) => {
...
function unescapeEntities(s: string): string {
  return String(s).replace(/&(#?\w+);/g, (m, name: string) => {
...
export function renderMarkdown(markdown: string, opts: RenderMarkdownOptions = {}): string {
  ...
  const ctx: RenderCtx = { c, width, links, footnotes: [], footnoteIndex: new Map() };
...
function renderBlocks(tokens: Token[], ctx: RenderCtx, indent: string): string {
  const parts: string[] = [];
...
function renderBlock(token: Token, ctx: RenderCtx, indent: string): string | null {
  const { c, width } = ctx;
  ...
  switch (token.type) {
    ...
    case "table":
      return renderTable(token as Tokens.Table, ctx, indent) + "\n";
    case "hr":
    ...
    case "def":
      return "";
    default: {
      const loose = token as { tokens?: Token[]; raw?: string };
      if (loose.tokens) return wrap(renderInline(loose.tokens, ctx), width, { indent });
      return loose.raw ? wrap(loose.raw, width, { indent }) : "";
    }
  }
}

function renderListItemBody(item: Tokens.ListItem, ctx: RenderCtx, sub: string): string {
  const parts: string[] = [];
...
function renderTable(token: Tokens.Table, ctx: RenderCtx, indent: string): string {
  const { c } = ctx;
  ...
  for (const t of tokens) {
    switch (t.type) {
      ...
      case "html":
        ...
        break;
      default: {
        const loose = t as { tokens?: Token[]; raw?: string; text?: string };
        out += loose.tokens ? renderInline(loose.tokens, ctx) : unescapeEntities(loose.raw ?? loose.text ?? "");
      }
    }
  }
  return out;
