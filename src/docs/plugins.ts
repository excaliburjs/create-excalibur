...
import { DEFAULT_LIMIT, FETCH_CONCURRENCY } from "./constants.ts";
import { pluginsCacheRoot, readJsonSync, writeFileAtomic, writeJsonAtomic } from "./cache.ts";
import { DocsNetworkError, DocsNotFoundError } from "./errors.ts";
import { mapConcurrent, requestJson } from "./http.ts";
import { plainText, type LocalHit } from "./local-index.ts";
import { parseFrontmatter, splitSections } from "./mdx.ts";
import type { DocsPage } from "./page.ts";
...
/** The subset of a full npm registry document we read. */
...
/** One MiniSearch document = one README section (with its final GitHub URL). */
interface IndexedPluginDoc {
  id: number;
  slug: string;
  anchor: string | null;
  title: string;
...
const PLUGIN_INDEX_OPTIONS: Options<IndexedPluginDoc> = {
...
function readmePath(short: string): string {
...
export function readPluginsManifest(): PluginsManifest | null {
  return readJsonSync<PluginsManifest>(pluginsManifestPath());
}

export function hasPluginIndex(): boolean {
  return fs.existsSync(indexFile()) && fs.existsSync(pluginsManifestPath());
}

export function pluginSlug(short: string): string {
  return `/plugins/${short}`;
}

export function readPluginReadme(short: string): string | null {
  try {
...
export async function writePluginReadme(short: string, contents: string): Promise<void> {
...
export function pluginEntryFromRegistryDoc(doc: RegistryDoc | null | undefined): { entry: PluginEntry; readme: string | null } | null {
  if (!doc) return null;
  const name = doc.name;
...
export async function discoverPlugins(): Promise<string[]> {
  try {
    const data = await requestJson<{ objects?: Array<{ package?: { name?: unknown } }> }>(
      `${NPM_REGISTRY}/-/v1/search?text=${encodeURIComponent("@excaliburjs")}&size=100`,
      { headers: NPM_HEADERS }
    );
    const names = (data?.objects ?? [])
      .map((o) => o?.package?.name)
      .filter((n): n is string => typeof n === "string" && n.startsWith(PLUGIN_SCOPE_PREFIX));
 ...
 */
export function buildPluginIndex(entries: PluginEntry[]): MiniSearch<IndexedPluginDoc> {
  const index = new MiniSearch(PLUGIN_INDEX_OPTIONS);
  const docs: IndexedPluginDoc[] = [];
  let id = 0;
...
export async function savePluginIndex(index: MiniSearch<IndexedPluginDoc>): Promise<void> {
...
const loadedIndexes = new Map<string, MiniSearch<IndexedPluginDoc>>();
function loadPluginIndex(): MiniSearch<IndexedPluginDoc> | null {
  const file = indexFile();
  if (loadedIndexes.has(file)) return loadedIndexes.get(file) ?? null;
  if (!fs.existsSync(file)) return null;
  const index = MiniSearch.loadJSON<IndexedPluginDoc>(fs.readFileSync(file, "utf8"), PLUGIN_INDEX_OPTIONS);
  loadedIndexes.set(file, index);
  return index;
}

/** Search the plugin READMEs. Same hit shape as the other sources, kind "plugin". */
export function searchPlugins(query: string, { limit = DEFAULT_LIMIT }: { limit?: number } = {}): LocalHit[] | null {
 ...
 */
export function loadPluginPage(slug: string): DocsPage {
...
export async function syncPlugins(
...
): Promise<{ plugins: number; warnings: string[] }> {
  const names = await discoverPlugins();
  const warnings: string[] = [];
  let done = 0;
  const results = await mapConcurrent(names, FETCH_CONCURRENCY, async (name): Promise<PluginEntry | null> => {
    try {
      const doc = await requestJson<RegistryDoc>(`${NPM_REGISTRY}/${name.replace("/", "%2F")}`, { headers: NPM_HEADERS });
  ...
  const plugins = results.filter((p): p is PluginEntry => p != null).sort((a, b) => a.name.localeCompare(b.name));
  if (!plugins.length) {
