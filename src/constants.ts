import { createNewGame } from "./flows/create-new-game.ts";
import { inspectGame } from "./flows/inspect-game.ts";
import { createSample } from "./flows/create-sample.ts";
import { searchDocs } from "./flows/docs.ts";
import { generateFlow } from "./flows/generate.ts";
import { mcpFlow } from "./flows/mcp.ts";
import { doctorFlow } from "./flows/doctor.ts";
import { upgradeFlow } from "./flows/upgrade.ts";

/** A CLI flow: receives the remaining argv after its command name. */
export type Flow = (argv?: string[]) => Promise<void> | void;
/** Registered flow keys, aliases included — derived from FLOWS below. */
export type FlowName = keyof typeof FLOWS;

export const FLOW_CHOICES: Array<{ name: string; value: FlowName }> = [
  {
    name: "Create a game",
    value: "create",
  ...
  {
    name: "Generate code (actor, scene, resource…)",
    value: "generate",
 ...
 */
export const QUIET_FLOWS: ReadonlySet<FlowName> = new Set<FlowName>(["docs", "mcp", "doctor", "upgrade", "up"]);
...
export interface ShowcaseDefinition {
  ...
  url: string;
  source: string;
  /** display label minted by the inspect flow */
  name?: string;
}
...
// NOTE: known duplicate entries in this data (typescript_excalibird twice,
// doubled Spritefusion) — the MCP create tool dedupes defensively; the data
// fix is tracked separately, so no uniqueness assert here yet.
export const SAMPLES: TemplateDefinition[] = [
  {
