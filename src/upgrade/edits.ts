...
import type { CheckResult, PlannedEdit, RunOutcome, UpgradeContext } from "./types.ts";
 ...
 */
export async function applyPlannedEdits(ctx: UpgradeContext, result: CheckResult): Promise<RunOutcome> {
  ...
  for (const [file, edits] of byFile) {
    ...
    try {
      ...
      skippedFiles.push({ file, reason: `could not apply edits (${error instanceof Error ? error.message : String(error)}) — left untouched` });
      continue;
    }
