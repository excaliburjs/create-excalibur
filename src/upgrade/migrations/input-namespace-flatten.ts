...
export const inputNamespaceFlatten: Migration = {
  ...
  check(ctx: UpgradeContext) {
    ...
    for (const { sf, text } of ctx.files) {
      ...
      const guard = (nameNode: TS.Node): boolean => {
      ...
      const handleNamespaceValue = (node: TS.PropertyAccessExpression): void => {
        const inner = node.expression as TS.PropertyAccessExpression; // ex.Input
      ...
      const handleNamedValue = (node: TS.PropertyAccessExpression): void => {
      ...
      const handleNamespaceType = (node: TS.QualifiedName): void => {
        const inner = node.left as TS.QualifiedName; // ex.Input
      ...
      const handleNamedType = (node: TS.QualifiedName): void => {
