 ...
 */
function topLevelObjectSpan(text: string, key: string): { start: number; end: number } | null {
  let depth = 0;
  let i = 0;
  while (i < text.length) {
    const ch = text[i];
 ...
 * manual instructions.
 */
export async function bumpExcaliburDep(projectDir: string, npmSpec: string): Promise<boolean> {
