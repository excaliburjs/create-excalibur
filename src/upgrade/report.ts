...
import type { UpgradeSummary } from "./run.ts";

const TYPE_BADGE: Record<string, string> = { auto: "auto", manual: "manual", notification: "note" };

/** Phase-1 plan preview: every applicable migration + sample hunks. */
export function renderPlan(summary: UpgradeSummary, { maxHunks = 8 }: { maxHunks?: number } = {}): void {
  const c = getChalk();
  terminal.blank();
  terminal.print(
...
export function renderResult(summary: UpgradeSummary): void {
  const c = getChalk();
  terminal.blank();
