...
export type TypeUtils = ReturnType<typeof createTypeUtils>;

export function createTypeUtils(ts: TsModule, checker: TS.TypeChecker, program: TS.Program) {
  /**
   ...
   */
  function isExcaliburSymbol(symbol: TS.Symbol | null | undefined): boolean {
   ...
   */
  function derivesFromExcalibur(type: TS.Type | null | undefined, nameSet: ReadonlySet<string>, seen: Set<TS.Type> = new Set()): boolean {
  ...
  function lineCol(sf: TS.SourceFile, node: TS.Node): { line: number; column: number } {
   ...
   */
  function chainContainsType(expr: TS.Expression, nameSet: ReadonlySet<string>): boolean {
  ...
  function enclosingExcaliburClass(node: TS.Node, nameSet: ReadonlySet<string>): TS.ClassDeclaration | null {
