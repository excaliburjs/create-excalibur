...
const uniform = (type) => ({ type, role: "uniform" });
const varying = (type) => ({ type, role: "varying" });
const attribute = (type) => ({ type, role: "attribute" });
 ...
 */
const MATERIAL_FRAGMENT_BUILTINS = new Map([
 ...
 */
const MATERIAL_VERTEX_BUILTINS = new Map([
 ...
 */
const SCREEN_SHADER_BUILTINS = new Map([
...
const declRegexCache = new Map();
function declRegex(reserved) {
...
export const noReservedUniforms = {
  ...
  create(ctx, sf) {
    ...
    const seen = new Set(); // same const reused for two materials → one finding
    ...
    const scanChunk = (chunk, reserved, construct) => {
      const text = blankComments(chunk.text);
      const re = declRegex(reserved);
      for (let m = re.exec(text); m; m = re.exec(text)) {
        const [, qual, middle, name, arraySuffix] = m;
    ...
    const scanExpr = (expr, reserved, construct) => {
      for (const chunk of resolveChunks(expr, 1)) scanChunk(chunk, reserved, construct);
    };

    const checkMaterialOptions = (optionsExpr) => {
    ...
    return {
      [ts.SyntaxKind.NewExpression](node) {
        ...
        const symbol = (type?.target ?? type)?.getSymbol?.();
