...
export const dontMutateSharedGraphics = {
  ...
  create(ctx, sf) {
    ...
    const tainted = new Set(); // ts.Symbol of tainted variables
    ...
    function isSeedCall(node) {
      return (
        ts.isCallExpression(node) &&
        ts.isPropertyAccessExpression(utils.unwrap(node.expression)) &&
        SEED_METHODS.has(utils.unwrap(node.expression).name.text)
      );
    }

    /** Does this expression evaluate to a cache-shared value? */
    function isTaintedValue(expr) {
    ...
    return {
      [ts.SyntaxKind.VariableDeclaration](node) {
