...
/** 1-based suppressed line → rules ("all" = every rule). */
export type IgnoreMap = Map<number, Set<string> | "all">;

/** Scan source text for ignore directives. */
export function collectIgnores(text: string): IgnoreMap {
...
export function isIgnored(ignores: IgnoreMap | undefined, line: number, rule: string): boolean {
 ...
 */
function templateLiteralLineStarts(text: string): boolean[] {
  const lines = text.split("\n");
  const startsInsideTemplate: boolean[] = new Array(lines.length);
 ...
 *
 */
export function insertIgnoreComments(
  projectDir: string,
  findings: Array<{ file: string; line: number; rule: string }>
): { modified: string[]; skipped: Array<{ file: string; line: number; rule: string }> } {
  ...
  const skipped: Array<{ file: string; line: number; rule: string }> = [];
