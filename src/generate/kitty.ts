...
/** Minimal stream surface the session needs (process.stdout satisfies it). */
export interface KittyStream {
  isTTY?: boolean;
...
function apc(keys: Record<string, string | number | undefined | null>, payload = ""): string {
 ...
 */
export function supportsKittyGraphics(stream: { isTTY?: boolean } = process.stdout, env: NodeJS.ProcessEnv = process.env): boolean {
 ...
 * Returns escape sequences (chunked at 4096 base64 bytes).
 */
export function transmitChunks(pngBuffer: Buffer, { id }: { id: number }): string[] {
 ...
 */
export function placeImage({ id, x, y, w, h, c, r }: { id: number; x?: number; y?: number; w?: number; h?: number; c?: number; r?: number }): string {
 ...
 */
export function fitCells(pxW: number, pxH: number, { maxCols = 40, maxRows = 12, cellW = 10, cellH = 20, upscale = false }: { maxCols?: number; maxRows?: number; cellW?: number; cellH?: number; upscale?: boolean } = {}): { c: number; r: number } {
 ...
 */
export function createKittySession(stream: KittyStream = process.stdout, env: NodeJS.ProcessEnv = process.env) {
  const enabled = supportsKittyGraphics(stream, env);
  let nextId = ((Date.now() & 0xffff) << 8) | (process.pid & 0xff);
  const transmitted = new Map<Buffer, number>(); // buffer -> image id
  function show(
    buf: Buffer | null | undefined,
    { sourceRect, maxCols = 40, maxRows = 12, upscale = false }: { sourceRect?: SourceRect; maxCols?: number; maxRows?: number; upscale?: boolean } = {}
  ): boolean {
...
export type KittySession = ReturnType<typeof createKittySession>;
