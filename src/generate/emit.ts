...
import { toCamelCase, toKebabCase } from "./names.ts";
import type {
  ActorModel,
  AnimationSpec,
...
/** One `name: expr` engine/actor option entry. */
export interface OptionEntry {
  name: string;
  expr: string;
}

function excaliburImportLine(names: Iterable<string>): string {
  const sorted = [...names].sort((a, b) => a.localeCompare(b));
  return `import { ${sorted.join(", ")} } from "excalibur";`;
}

/** Actor / Label constructor option entries + the excalibur imports they need. */
function actorOptionEntries(model: ActorModel, imports: Set<string>): string[] {
...
export function emitActorFile(model: ActorModel, { resourcesSpecifier = "./resources" }: { resourcesSpecifier?: string } = {}): string {
...
export function emitLabelFile(model: LabelModel): string {
...
const SCENE_LIFECYCLE: Record<string, { signature: string; imports: string[]; comment: string }> = {
  onInitialize: {
...
export function emitSceneFile(model: SceneModel): string {
...
export function emitResourcesFile(): string {
  return [
...
export function emitResourceExpr(model: ResourceExprModel): { expr: string; excaliburImports: string[] } {
 ...
 */
export function emitSpriteSheetConst(
  model: Pick<SpriteSheetModel, "name" | "image" | "grid" | "spacing">
): { text: string; excaliburImports: string[] } {
  const g = model.grid;
  const lines: string[] = [];
  ...
  const sp = model.spacing ?? { margin: null, originOffset: null };
...
export function emitAnimationConst(
  model: AnimationSpec & { sheetName: string }
): { text: string; excaliburImports: string[] } {
  const lines: string[] = [];
...
export function engineOptionEntries(
  options: EngineOptionsModel | null | undefined,
  imports: Set<string> = new Set()
): { entries: OptionEntry[]; imports: Set<string> } {
 ...
 */
export function actorArgEntries(
  options: ActorArgsOptionsModel | null | undefined,
  imports: Set<string> = new Set()
): { entries: OptionEntry[]; imports: Set<string> } {
  const entries: OptionEntry[] = [];
  const o = options ?? {};
...
export function emitMainFile(
  model: { options?: EngineOptionsModel | null; scenes?: Array<{ key: string; className: string; specifier: string }> | null },
  { hasResources = false }: { hasResources?: boolean } = {}
): string {
...
function graphicUvPrelude(pixelArt: boolean): { uniforms: string; helpers: string; uv: string } {
  if (pixelArt) {
    return {
...
function tintGlsl(pixelArt: boolean): string {
...
function outlineGlsl(pixelArt: boolean): string {
  const p = graphicUvPrelude(pixelArt);
  return `#version 300 es
precision mediump float;
...
export function materialNames(className: string): { factoryName: string; sourceConst: string; materialName: string } {
...
function escapeTemplateLiteral(src: string): string {
  return src.replace(/\\/g, "\\\\").replace(/`/g, "\\`").replace(/\$\{/g, "\\${");
}

export function emitMaterialFile(model: MaterialModel): string {
  const names = materialNames(model.className);
  const template = MATERIAL_TEMPLATES[model.template ?? "tint"] ?? MATERIAL_TEMPLATES.tint;
