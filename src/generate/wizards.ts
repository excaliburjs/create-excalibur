...
import { toCamelCase, toKebabCase, toPascalCase, isValidIdentifier } from "./names.ts";
import { createTsEditor } from "./ts-edit.ts";
import { MATERIAL_TEMPLATES, SCENE_LIFECYCLE_METHODS } from "./emit.ts";
import type { Project, ProjectScene, ProjectActor } from "./project.ts";
import type {
  ActorAdvancedOptions,
  ActorArgsOptionsModel,
  ActorModel,
  ColliderModel,
  EngineModel,
  EngineOptionsModel,
  EnginePhysicsOptions,
  GraphicModel,
  LabelFontModel,
  LabelModel,
  MaterialModel,
...
/** What every wizard receives: the analyzed project + pre-parsed CLI args. */
...
export async function resolveName(ctx: WizardContext, kindLabel: string): Promise<{ className: string; fileName: string }> {
...
export async function pickScene(ctx: WizardContext, message = ""): Promise<ProjectScene | null> {
  const c = getChalk();
  ...
  return select<ProjectScene | null>({
    message,
    choices: [
      ...scenes.map((s) => ({
        name: s.className,
        value: s as ProjectScene | null,
...
export async function pickActor(ctx: WizardContext, message = ""): Promise<ProjectActor | null> {
  const c = getChalk();
  ...
  return select<ProjectActor | null>({
    message,
    choices: [
      ...actors.map((a) => ({
        name: a.className,
        value: a as ProjectActor | null,
...
export async function actorWizard(ctx: WizardContext): Promise<ActorModel & { kind: "actor" }> {
  const { className, fileName } = await resolveName(ctx, "Actor");

  const geometry = await select<"box" | "circle" | "custom" | "none">({
  ...
  let collider: ColliderModel;
  if (geometry === "box") {
    collider = {
      type: "box",
      width: (await number({ message: "Width (px):", default: 100 })) ?? 100,
      height: (await number({ message: "Height (px):", default: 100 })) ?? 100,
    };
  } else if (geometry === "circle") {
    collider = { type: "circle", radius: (await number({ message: "Radius (px):", default: 50 })) ?? 50 };
  } else {
    collider = { type: geometry };
  }

  const graphicChoices: Array<{ name: string; value: "color" | "sprite" | "none" }> = [];
  ...
  const graphicType =
    graphicChoices.length === 1 ? "none" : await select({ message: "Default graphic:", choices: graphicChoices });
  let graphic: GraphicModel;
  ...
  const advanced: ActorAdvancedOptions = {};
  if (await confirm({ message: "More options? (position, z, anchor, coordinate plane…)", default: false })) {
    ...
    if (picks.includes("pos")) {
      advanced.pos = {
        x: (await number({ message: "x:", default: 0 })) ?? 0,
        y: (await number({ message: "y:", default: 0 })) ?? 0,
      };
    }
    if (picks.includes("z")) advanced.z = (await number({ message: "z index:", default: 0 })) ?? 0;
    if (picks.includes("anchor")) {
      advanced.anchor = await select<"center" | "topLeft">({
        message: "Anchor:",
        choices: [
    ...
    if (picks.includes("coordPlane")) {
      advanced.coordPlane = await select<"World" | "Screen">({
    ...
    if (picks.includes("rotation")) advanced.rotation = (await number({ message: "Rotation (radians):", default: 0 })) ?? 0;
...
export async function labelWizard(ctx: WizardContext): Promise<LabelModel & { kind: "label" }> {
  ...
  if (await confirm({ message: "Set a position?", default: true })) {
    pos = {
      x: (await number({ message: "x:", default: 10 })) ?? 10,
      y: (await number({ message: "y:", default: 10 })) ?? 10,
    };
  }
  const targetScene = await pickScene(ctx, "Add it to a scene's onInitialize?");
...
export async function sceneWizard(ctx: WizardContext): Promise<SceneModel & { kind: "scene" }> {
...
export const RESOURCE_TYPES: Record<string, ResourceTypeSpec> = {
...
export function scanFiles(root: string, exts: string[] | null | undefined, cap = 2000, depth = 8): string[] {
  const out: string[] = [];
 ...
 */
export async function pickAssetFile(ctx: WizardContext, exts: string[] | null | undefined, message = "File (under public/):"): Promise<string> {
  const c = getChalk();
  const files = scanFiles(ctx.project.publicDir, exts);
  const MANUAL = Symbol("manual");
  type Choice = { name: string; value: string | typeof MANUAL; disabled?: boolean };
  const toChoice = (f: string): Choice => {
    const relPath = path.relative(ctx.project.publicDir, f).split(path.sep).join("/");
    return { name: relPath, value: `./${relPath}` };
  };
  let assetPath = await search<string | typeof MANUAL>({
    message,
    source: async (term) => {
      const lower = (term ?? "").toLowerCase();
      const matches: Choice[] = files.filter((f) => f.toLowerCase().includes(lower)).slice(0, 20).map(toChoice);
...
export async function resourceWizard(ctx: WizardContext): Promise<ResourceModel & { kind: "resource" }> {
  ...
  const model: ResourceModel & { kind: "resource" } = { kind: "resource", key, resourceClass: spec.class, assetPath };
  if (type === "image") {
  ...
  type Target = { root?: boolean; scene?: ProjectScene };
  const targets: Array<{ name: string; value: Target }> = [
    { name: "Root loader (resources.ts — loads at boot)", value: { root: true } },
  ];
...
export async function materialWizard(ctx: WizardContext): Promise<MaterialModel & { kind: "material" }> {
  const c = getChalk();
...
export async function updateActorWizard(ctx: WizardContext): Promise<UpdateActorModel & { kind: "update-actor" }> {
  const { project } = ctx;
  ...
  try {
    ...
    for (const prop of lit.properties) {
      ...
      if (name && project.ts.isPropertyAssignment(prop)) {
        current.set(name, text.slice(prop.initializer.getStart(sf), prop.initializer.end));
      }
    }
  } catch {
  ...
  const label = (name: string) => (current.has(name) ? `${name} (currently ${current.get(name)})` : `${name} (not set)`);
  const picks = await checkbox({
  ...
  const options: ActorArgsOptionsModel = {};
  const remove: string[] = [];
  if (picks.includes("pos")) {
    options.pos = {
      x: (await number({ message: "x:", default: 0 })) ?? 0,
  ...
  if (picks.includes("size")) {
    options.width = (await number({ message: "width:", default: 100 })) ?? 100;
    options.height = (await number({ message: "height:", default: 100 })) ?? 100;
    if (current.has("radius")) remove.push("radius");
  }
  if (picks.includes("radius")) {
    options.radius = (await number({ message: "radius:", default: 50 })) ?? 50;
  ...
  if (picks.includes("anchor")) {
    options.anchor = await select<"center" | "topLeft">({
      message: "anchor:",
      choices: [
        { name: "Center (0.5, 0.5) — the default", value: "center" },
        { name: "Top-left (0, 0)", value: "topLeft" },
  ...
  if (picks.includes("coordPlane")) {
    options.coordPlane = await select<"World" | "Screen">({
      message: "coordPlane:",
      choices: [
        { name: "World (moves with the camera — the default)", value: "World" },
        { name: "Screen (fixed to the viewport, e.g. UI)", value: "Screen" },
  ...
  if (picks.includes("rotation")) options.rotation = (await number({ message: "rotation (radians):", default: 0 })) ?? 0;
  if (picks.includes("z")) options.z = (await number({ message: "z index:", default: 0 })) ?? 0;
...
export function detectPixelArt(project: Project): boolean {
  if (!project.mainFile) return false;
  try {
...
export async function engineWizard(ctx: WizardContext): Promise<EngineModel & { kind: "engine" }> {
  const { project } = ctx;
  const options: EngineOptionsModel = {};
  const remove: string[] = [];
  ...
  if (picks.includes("size")) {
    options.width = (await number({ message: "width:", default: 800 })) ?? 800;
    options.height = (await number({ message: "height:", default: 600 })) ?? 600;
  ...
  if (picks.includes("fixedUpdateFps")) options.fixedUpdateFps = (await number({ message: "fixedUpdateFps:", default: 60 })) ?? 60;
  if (picks.includes("physics")) {
    const physics: EnginePhysicsOptions = {};
    ...
    if (await confirm({ message: "Set gravity?", default: false })) {
      physics.gravity = {
        x: (await number({ message: "gravity x:", default: 0 })) ?? 0,
        y: (await number({ message: "gravity y:", default: 800 })) ?? 800,
      };
    }
    options.physics = physics;
  }

  const model: EngineModel & { kind: "engine" } = { kind: "engine", options, remove };
  if (!project.mainFile && project.scenes.length) {
    const chosen = await checkbox<ProjectScene>({
