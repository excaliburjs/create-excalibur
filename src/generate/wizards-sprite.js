...
import { toPascalCase, isValidIdentifier } from "./names.js";
import { createTsEditor } from "./ts-edit.js";
import { detectPixelArt, pickActor, pickAssetFile, RESOURCE_TYPES } from "./wizards.js";
import { parseImageSize } from "./image.js";
import { createKittySession } from "./kitty.js";
...
export function assetDiskPath(project, assetPath) {
  return path.join(project.publicDir, assetPath.replace(/^\.\//, ""));
}

function readAsset(project, assetPath) {
  try {
 ...
 * @returns {{ grid: object | null, warnings: string[] }}
 */
export function resolveGrid({ dimensions, rows = null, columns = null, spriteWidth = null, spriteHeight = null, margin, originOffset }) {
  const warnings = [];
  ...
  let w = spriteWidth;
  let h = spriteHeight;
  const deriveSize = (axis, total, offset, gap, count) => {
  ...
  const deriveCount = (axis, total, offset, gap, size) => {
  ...
  if (![r, c, w, h].every((v) => Number.isFinite(v) && v >= 1)) {
...
function collectModuleConsts(project, file) {
  const names = new Set();
...
async function resolveConstName({ preset, message, defaultValue, suffix, taken }) {
 ...
 * @returns {{ name: string, frames: {x,y,duration}[], strategy: string }}
 */
async function animationSubFlow(sheetInfo, session, buf, taken, presetName = null) {
  const c = getChalk();
  const { grid } = sheetInfo;
  const spacing = sheetInfo.spacing ?? {};
  ...
  const defaultMs = await number({ message: "Default frame duration (ms):", default: 100, min: 1 });
...
export async function spritesheetWizard(ctx) {
  const c = getChalk();
  const { project } = ctx;
  ...
  try {
    ...
    if (mode === "counts" || mode === "all") {
      partial.columns = await number({ message: "Columns:", default: 1, min: 1 });
      partial.rows = await number({ message: "Rows:", default: 1, min: 1 });
    }
    if (mode === "size" || mode === "all") {
      partial.spriteWidth = await number({ message: "Sprite width (px):", default: 16, min: 1 });
      partial.spriteHeight = await number({ message: "Sprite height (px):", default: 16, min: 1 });
    }
    const spacing = {
      margin: {
        x: await number({ message: "Margin between sprites — x (px):", default: 0, min: 0 }),
        y: await number({ message: "Margin between sprites — y (px):", default: 0, min: 0 }),
      },
      originOffset: {
        x: await number({ message: "Origin offset from the top-left — x (px):", default: 0, min: 0 }),
        y: await number({ message: "Origin offset from the top-left — y (px):", default: 0, min: 0 }),
      },
    };
...
export async function animationWizard(ctx) {
  const c = getChalk();
  const { project } = ctx;
