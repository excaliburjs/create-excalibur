...
import { createGalaxy } from '../particles/galaxy.particles';
...
export class LevelScene extends Scene {
  ...
  init(engine: Engine) {
    ...
    paddle.on('postupdate', () => {
      const hitLimits = {
        screenLeft: paddle.pos.x - paddle.width / 2 < 0,
      ...
      if (hitLimits.screenRight) {
        paddle.pos.x = engine.drawWidth - paddle.width / 2;
      }
    });
  ...
  createGalaxyParticles(engine: Engine) {
    return createGalaxy({
  ...
  createBricks() {
    ...
    this.bricksSetup.forEach((row: any[], rowIndex: number) => {
