...
import { parseUpgradeArgs, UPGRADE_USAGE } from "../upgrade/args.ts";
import { runUpgrade } from "../upgrade/run.ts";
import { renderPlan, renderResult } from "../upgrade/report.ts";
import { GenerateError } from "../generate/errors.ts";

function reportUpgradeError(error: GenerateError): void {
  const c = getChalk();
  terminal.blank();
 ...
 */
export async function upgradeFlow(argv: string[] = []): Promise<void> {
