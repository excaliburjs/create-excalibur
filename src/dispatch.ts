import { FLOWS, type FlowName } from "./constants.ts";
...
export type Invocation =
  | { kind: "help" }
  | { kind: "flow"; flow: FlowName; rest: string[] }
  | { kind: "create"; name: string }
  | { kind: "unknown"; command: string }
  | { kind: "menu" };

export function resolveInvocation({ binName, argv }: { binName: string; argv: string[] }): Invocation {
  ...
  if (command && Object.hasOwn(FLOWS, command)) {
    return { kind: "flow", flow: command as FlowName, rest };
  }
